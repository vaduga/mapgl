...
7. Use a different **Edge ID** for each repeated **Vertex A -> Vertex B** link that should remain parallel.
8. Reuse one **Edge ID** only when consecutive, datasource-ordered rows describe continuous portions of the same route.
...
When several **Markers and links** layers are configured, Mapgl composes them
in data-layer order into one graph. Each layer keeps its own **Data** matcher,
field mappings, styles, and LayerSwitcher entry. A link in one layer can target
a node supplied by another layer because link validation runs after nodes from
all configured graph layers are collected.
...
# One routed graph link
Vertex A=A, Vertex B=["A", "B", "C"], Edge ID=route-1

# One routed Geo link through a longitude/latitude waypoint
Vertex A=A, Vertex B=["A", [15, 55], "B"], Edge ID=geo-route-1
```

If an intermediate node ID does not exist, Mapgl skips that waypoint and keeps
the route between the remaining valid nodes. Missing source or final target
nodes still prevent the link from being created.
...
For service dependency graphs and trace-like data, use **Vertex B** as the service path and **Edge ID** as the trace or branch key. Rows that share an **Edge ID** must be consecutive, remain in datasource order, and connect end-to-start. Mapgl displays them as one routed link, while styling and tooltip fields from each row remain associated with the corresponding route portion.
...
```text
Vertex A        Vertex B            Edge ID   duration_ms
api-gateway     payment-service     trace-42  18
payment-service postgres-primary    trace-42  41
```

The two rows are adjacent and the second begins where the first ends, so `trace-42` is displayed as one ordered route. Fields such as duration or cost remain available for the matching route portion in the edge tooltip.
...
- matching groups can mark matching edges as dashed
...
- **Hide diagnostic messages**: suppress graph-frame diagnostic alerts in dashboard and panel edit mode.
