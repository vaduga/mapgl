...
import {Geometry as MyGeometry, LineString as MyLineString} from 'geojson'
...
export function toLineString(field: Field<MyGeometry | undefined>): MyLineString {
  const coords: number[][] = [];
  ...
  const LS= {type: 'LineString' as 'LineString', coordinates: []};
  return LS
}
