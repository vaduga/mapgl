...
import {
    anyToNumber,
    dateTimeParse, DecimalCount,
    DisplayProcessor, DisplayValue,
    Field,
    FieldType, FormattedValue, getFieldTypeFromValue, getValueFormat,
    GrafanaTheme2,
    isBooleanUnit,
    KeyValue,
    TimeZone, toUtc
...
export function getDisplayProcessor(options?: DisplayProcessorOptions): DisplayProcessor {
    ...
    return (value: unknown, capacity, adjacentDecimals?: DecimalCount) => {
        ...
        if (!Number.isNaN(numeric)) {
            ...
            if (color == null) {
                const scaleResult = scaleFunc(numeric, capacity);
        ...
        if (!color) {
            const scaleResult = scaleFunc(-Infinity, capacity);
            color = scaleResult.color;
            percent = scaleResult.percent;
        }
