...
export interface ExtendMapLayerHandler<TConfig = any> {
  ...
  /**
   * Generic query-data lifecycle for non-graph layers.
   * Graph layers are coordinated by the graph frame pipeline instead.
   */
...
export interface ExtendMapLayerRegistryItem<TConfig = ExtendMapLayerOptions> extends RegistryItemWithOptions {
  ...
  /**
   * Show the query-data selector even when the layer uses a dedicated
   * coordinator instead of the generic handler update lifecycle.
   */
  usesQueryData?: boolean;
  /**
