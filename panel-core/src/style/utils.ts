...
export function resolveStyleConfigState(cfg: StyleConfig | undefined, theme: GrafanaTheme2): StyleConfigState {
  if (!cfg) {
  ...
  const state: StyleConfigState = {
    ...
    fields,
    base: {
      color: theme.visualization.getColorByName(cfg.color?.fixed ?? defaultStyleConfig.color.fixed),
  ...
  if (cfg.arrow !== undefined) {
    state.base.arrow = cfg.arrow;
  }
  if (cfg.useGroups !== undefined) {
...
export async function getStyleConfigState(cfg?: StyleConfig): Promise<StyleConfigState> {
  return resolveStyleConfigState(cfg, config.theme2);
}
