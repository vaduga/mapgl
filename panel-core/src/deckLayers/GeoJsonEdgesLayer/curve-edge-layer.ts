...
export type CurveEdgeBinaryData<DataT = any> = {
  ...
  attributes: {
    ...
    getSkip?: { value: Uint8Array; size: 1 };
  };
  features: DataT[];
...
const defaultProps: DefaultProps<CurveEdgeLayerProps<any>> = {
  ...
  skipVisibleMaxDepth: { type: 'number', value: -1 },
  ...
  getSkip: { type: 'accessor', value: (d: CurveEdgeSegment) => Number(Boolean((d.feature as any)?.skip)) },
};
...
void main(void) {
  if (vSkip > 0.5 && vHighlightDepth > curve.skipVisibleMaxDepth) {
    discard;
  }
