...
export class GraphHighlighter {
  ...
  setGraph(graph: Graph, opts?: { force?: boolean; edgeIndex?: GraphEdgeIndex }) {
    ...
    const recordEdges = new Set<Edge>();
    for (let recordRef = 0; recordRef < (edgeIndex?.recordCount ?? 0); recordRef++) {
      const edges = [...edgeIndex!.recordEdges(recordRef)];
      if (!edges.length) {
        continue;
      }
  ...
  private addDirectedAdjacent(
    source: Node,
    target: Node,
    edges: Edge[],
    edgeKeys = edges.map((edge) => getEdgeKey(edge))
  ) {
