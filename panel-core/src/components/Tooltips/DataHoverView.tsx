...
const Trans = ({ children }: { i18nKey?: string; children?: React.ReactNode }) => <>{children}</>;

function renderValue(value: string): string | React.ReactNode {
  if (/^https?:\/\//.test(value)) {
    return (
...
export function getDisplayValuesAndLinks(
  ...
  for (const field of orderedVisibleFields) {
    ...
    getFieldDisplayLinksCompat(
      field,
      rowIndex,
      scopedVars && replaceVariables ? { frame: data, scopedVars, replaceVariables } : undefined
    ).forEach((link) => {
