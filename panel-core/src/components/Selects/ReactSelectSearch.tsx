...
type MapRefProps = {
  ...
  selectGotoHandler?: (a: Partial<handlerProps>) => Promise<void> | undefined;
  rootStore: ReactSelectSearchRootStore;
  value?: string;
  placeholder?: string;
...
const ReactSelectSearch: FC<MapRefProps> = ({
  ...
  const { pointStore, graph, pId } = rootStore;
    ...
    <div className={s.select}>
      <ComboboxCompat
        ...
        onChange={(v) => {
          ...
          if (selectHandler) {
            void selectHandler({ value: selected.nodeId });
            return;
          }

          void selectGotoHandler?.({
            pId,
