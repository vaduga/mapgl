...
import { InlineField, InlineFieldRow, Tooltip, useStyles2 } from '@grafana/ui';
import { observer } from 'mobx-react-lite';
import React, { useState } from 'react';

import type { Graph } from '../graph/main';
import { selectGotoHandler } from '../utils';
import ReactSelectSearch from './Selects/ReactSelectSearch';

export interface MenuActionContext<TRootStore = unknown, TPanel = unknown, TOptions = unknown> {
  data: PanelData;
  eventBus?: EventBus;
  options: TOptions;
  panel: TPanel;
...
export interface MenuAction<TRootStore = unknown, TPanel = unknown, TOptions = unknown> {
  id: string;
  render: (context: MenuActionContext<TRootStore, TPanel, TOptions>) => React.ReactNode;
}

export interface MenuProps<TRootStore = unknown, TPanel = unknown, TOptions = unknown> {
  actions?: Array<MenuAction<TRootStore, TPanel, TOptions>>;
  data: PanelData;
  eventBus?: EventBus;
  options: TOptions;
  panel: TPanel;
...
interface RootStoreWithSelection {
  pointStore: {
...
const Menu = <TRootStore extends RootStoreWithSelection = RootStoreWithSelection, TPanel = unknown, TOptions = unknown>({
  actions = [],
  data,
  eventBus,
  options,
  panel,
  rootStore,
}: MenuProps<TRootStore, TPanel, TOptions>) => {
  const s = useStyles2(getStyles);
  const [isSearchOpen, setIsSearchOpen] = useState(false);
  const isSearchVisible = isSearchOpen || Boolean(rootStore.pointStore?.getSelectedNode);
  const actionContext: MenuActionContext<TRootStore, TPanel, TOptions> = {
    data,
    eventBus,
    options,
    panel,
  ...
  return (
    <div className={s.myMenu}>
      <InlineFieldRow className={s.inlineRow}>
        ...
        {actions.map((action) => (
          <React.Fragment key={action.id}>{action.render(actionContext)}</React.Fragment>
        ))}
      </InlineFieldRow>
    </div>
  );
