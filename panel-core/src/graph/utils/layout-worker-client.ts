...
type PanelLike = {
  ...
  graphEngine?: {
    setPositions?: (positions: Float64Array, offset: number) => void;
    setShifts?: (shifts: Array<{ ns: string; shift: [number, number] }>) => void;
  };
...
function syncExternalLayoutState(panel: PanelLike): void {
  panel.graphEngine?.setPositions?.(panel.positions, 0);

  const shifts = Object.entries(panel.layerShift ?? {}).map(([ns, [x, y]]) => ({
    ns,
    shift: [x | 0, y | 0] as [number, number],
  }));
  if (shifts.length) {
    panel.graphEngine?.setShifts?.(shifts);
  }
}
