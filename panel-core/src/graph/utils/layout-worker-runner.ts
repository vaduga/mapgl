...
function buildGraphFromSnapshot(request: LayoutRequest): Graph {
  ...
  for (const item of request.nodes) {
    ...
    geomNode.boundaryCurve = CurveFactory.mkCircle(item.radius ?? DEFAULT_LAYOUT_NODE_RADIUS, new Point(0, 0));
...
function addCurveSegments(
  ...
  if (curve instanceof LineSegment) {
    pushSegment(segments, CURVE_TYPE_LINE, [...pointToArray(curve.start), ...pointToArray(curve.end), 0, 0, 0, 0], 1);
    return;
  }
