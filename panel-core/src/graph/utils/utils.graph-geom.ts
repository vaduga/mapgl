...
export function getEdgesGeometry(panel: any) {
  ...
  graphEdgeIndex.wasm2Edges.forEach((edges, heIdx) => {
    ...
    edges.forEach((edge, fragIdx) => {
      ...
      const renderDecision = renderDecisions.get(key);
...
function applyEdgeRenderDecisions({
  ...
  graphEdgeIndex: any;
  skipFrags: Set<string>;
  geomOverride: Map<FragKey, Position[]>;
  renderDecisions: Map<FragKey, EdgeRenderDecision>;
  applyGeometryOverrides: boolean;
}) {
  decisions.forEach((decision) => {
    const lineId = decision.lineId;

    if (decision.tiltDistance !== undefined) {
      decision.edge.setTiltDist(decision.tiltDistance, decision.isOutgoingTilt ?? false);
    }

    if (lineId === undefined || decision.fragmentIndex === undefined) {
      return;
    }

    const { parPath } = graphEdgeIndex.wasm2Edges[lineId]?.at(0)?.data || {};
    if (!parPath) {
      return;
    }

    const key = fragKey(lineId, decision.fragmentIndex);
    renderDecisions.set(key, decision);
    ...
    if (decision.type === 'offset' && decision.coordinates && applyGeometryOverrides) {
      geomOverride.set(key, decision.coordinates);
    }
  });
}
