...
import { SvgTintMode } from '../../../deckLayers/utils/svg';
...
const getRulesWithOverridesCounts = (properties, groups, theme, locField, locName, isFixed) => {
  const matches: Array<Rule & extRule> = [];
  (groups as Rule[]).forEach((g, idx) => {
    const [oCount, hasAllValuesMatch, hasLocMatch, hasThrMatch] = countMatchingKeysAndValues(
      properties,
      g.overrides,
      theme,
      locField,
    ...
    if (hasAllValuesMatch || (g.isEph && hasThrMatch)) {
      // && (oCount > 0 || (hasThrMatch && g.isEph)) /// second condition for no set groups
      const group = {
        ...g,
        oCount,
        hasLocMatch,
